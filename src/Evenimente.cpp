...
void InspectieSanitara::Executa(ContextEveniment& ctx) {
    std::cout << "\n[!] ALERTA: Inspectorii sanitari au intrat in restaurant!\n";
    double murdarie = ctx.r.getGradMurdarie();
    std::cout << "Nivel actual de murdarie: " << murdarie << "%\n";

    if (murdarie > 50.0) {
        double amenda = 500.0;
        ctx.profitTotal -= amenda;
...
void PanaCurent::Executa(ContextEveniment& ctx) {
...
void Influencer::Executa(ContextEveniment& ctx) {
    std::cout << "\n[!] EVENIMENT: Unul dintre clienti este un INFLUENCER faimos!\n";

    if (ctx.areLocLaMasa) {
        std::cout << "Influencerul cere toata comanda GRATIS (" << ctx.costComanda << " RON) pentru un Story.\n";
        ...
        if (opt == 1) {
            ...
            ctx.r.modificaPublicitate(1);
            std::cout << "Ai acceptat. Ai platit nota din buzunar, dar Influencerul te-a promovat! Nivel Publicitate: " << ctx.r.getNivelPublicitate() << "\n";
        } else {
            ctx.r.modificaPublicitate(-1);
            std::cout << "Ai refuzat. Influencerul a platit, dar a lasat un review oribil. Nivel Publicitate: " << ctx.r.getNivelPublicitate() << "\n";
        }
    } else {
...
void CriticCulinar::Executa(ContextEveniment& ctx) {
